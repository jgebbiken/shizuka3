...
struct whisper_all_packet_func
{
	const char * m_str;

	whisper_all_packet_func(const char * str) : m_str(str)
	{
	}
	...
	void operator () (LPDESC d)
	{
		...
		strlcpy(pack.szNameFrom, "Admin" , sizeof(pack.szNameFrom));
...
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
	std::for_each(c_ref_set.begin(), c_ref_set.end(), whisper_all_packet_func(c_pszBuf));
}
...
{
	TPacketGGNotice p;
	p.bHeader = HEADER_GG_WHISPER_ALL;
	p.lSize = strlen(c_pszBuf) + 1;
