...
//		BindDesc(NULL);
	...
	LPBROWSER browser = m_pkBrowser;
	if (browser)
	{
		SetBrowser(NULL); // 안해도 되지만 안전하게.
	}
	...
	event_cancel(&m_pkDungeonBrowserEvent);
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
	...
	if(m_pkBrowser)
	{
		m_pkBrowser->__RemoveViewer(this);
		CDungeonBrowserManager::instance().P2PLogout(this);
	}
			...
			if (amount > 0)
			{
				...
				switch (val)
				{
					...
					case 3:
						...
						if (GetLevel() < 150) PointChange(POINT_STAT, 1);
						break;
...
void CHARACTER::SetBrowser(LPBROWSER pkBrowser)
{
	if (pkBrowser == m_pkBrowser)
		return;

	if (pkBrowser && m_pkBrowser)
		sys_err("%s is trying to reassigning browser (current %p, new party %p)", GetName(), get_pointer(m_pkBrowser), get_pointer(m_pkBrowser));

	sys_log(1, "Browser set to %p", get_pointer(m_pkBrowser));

	m_pkBrowser = pkBrowser;
}
...
void CHARACTER::ExitToSavedLocation()
{
	...
	if (m_pkDungeon->IsDungeonTeam()){
		WarpSet(GetQuestFlag("savemapx"), GetQuestFlag("savemapy"), GetQuestFlag("savemapindex"));
