...
//	HEADER_CG_RUNUP_MATRIX_ANSWER	= 201,
	...
	HEADER_CG_BROWSER_ADD = 207,
	HEADER_CG_BROWSER_REMOVE = 208,
	HEADER_CG_BROWSER_CREATE = 209,
	HEADER_CG_BROWSER_DESTROY = 210,
	HEADER_CG_BROWSER_UPDATE = 211,
	...
	HEADER_GG_WHISPER_ALL = 30,

	HEADER_GG_BROWSER_TOCOPY = 31,
	HEADER_GG_BROWSER_GIVEWARP = 32,
	HEADER_GG_BROWSER_SAVELOC = 33,
};
...
typedef struct SPacketGGBrowser
{
	BYTE	bHeader;
	BYTE	bSubHeader;
	DWORD	dwLeaderID;
...
typedef struct SPacketGGBrowserChat
{
	BYTE	bHeader;
	BYTE	bSubHeader;
	DWORD	dwLeaderID;
	char	szText[CHAT_MAX_LEN + 1];
} TPacketGGBrowserChat;

typedef struct SPacketGGBrowserGotoCopyG
{
	BYTE	bHeader;
	BYTE	bSubHeader;
	DWORD	dwLeaderID;
} TPacketGGBrowserGotoCopyG;

typedef struct SPacketGGBrowserGotoCopy
{
	BYTE	bHeader;
	DWORD	dwLeaderID;
} TPacketGGBrowserGotoCopy;

typedef struct SPacketGGBrowserGiveWarp
{
	BYTE	bHeader;
	DWORD	dwLeaderID;
	long	map_index;
	long	mapx;
	long	mapy;
} TPacketGGBrowserGiveWarp;

typedef struct SPacketGGBrowserExitLoc
{
	BYTE	bHeader;
	DWORD	dwLeaderID;
	DWORD	player_id;
	...
	long	mapx;
	long	mapy;
} TPacketGGBrowserExitLoc;

typedef struct SPacketGGBrowserJoin
{
	BYTE	bHeader;
	BYTE	bSubHeader;
	DWORD	dwLeaderID;

} TPacketGGBrowserJoin;
...
enum EDungeonBrowserSubHeader
{
	BROWSER_SUBHEADER_GG_CHAT,
	BROWSER_SUBHEADER_GG_GOTOCOPY,
	BROWSER_SUBHEADER_GG_SAVELOC,
	BROWSER_SUBHEADER_GG_GIVEWARP,
	
};

enum EDungeonBrowserGCSubHeader
{
	BROWSER_SUBHEADER_GC_ADD,
	BROWSER_SUBHEADER_GC_REMOVE,
	BROWSER_SUBHEADER_GC_CREATE,
	BROWSER_SUBHEADER_GC_DESTROY,
	BROWSER_SUBHEADER_GC_UPDATE,
};
...
typedef struct command_browser_add
{
	BYTE	header;
	DWORD	dwPID;
} TPacketCGBrowserAdd;

typedef struct command_browser_destroy
{
	BYTE	header;
	DWORD	dwPID;
} TPacketCGBrowserDestroy;

typedef struct command_browser_create
{
	BYTE	header;
	int	bGameIndex;
} TPacketCGBrowserCreate;

typedef struct command_browser_update
{
	BYTE	header;
	BYTE	bState;
} TPacketCGBrowserUpdate;
...
typedef struct packet_browser_team
{
	BYTE	header;
	WORD	size;
	BYTE	subheader;
} TPacketGCBrowserTeam;
...
typedef struct packet_browser_add
{
	BYTE	header;
	DWORD	pid;
	char	name[CHARACTER_NAME_MAX_LEN + 1];
