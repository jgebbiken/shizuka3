...
void CInputDB::Boot(const char* data)
{
	...
	sys_log(0, "sizeof(TBrowserManagerInitial) = %d", sizeof(TBrowserManagerInitial));
	//ADMIN_MANAGER
	...
	if (decode_2bytes(data) != sizeof(TBrowserManagerInitial))
	{
	...
	size = decode_2bytes(data);
	data += 2;
	sys_log(0, "BOOT: DUNGEON_BROWSER: %d", size);

	if (size)
	{
		TBrowserManagerInitial* p = (TBrowserManagerInitial*)data;
		for (int i = 0; i < size; ++i, ++p)
		{
			sys_log(0, "BOOT: TBrowserManagerInitial: %d", p->map_index);
			CDungeonBrowserManager::Instance().DB_CreateTeam(p->dwLeaderID, p->bGameIndex, p->map_index);
			CDungeonBrowserManager::Instance().LoadMember(p->dwLeaderID);
		}

		data += sizeof(TBrowserManagerInitial)* size;
	}
...
int CInputDB::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	case HEADER_DG_BROWSER_TEAM_UPDATE:
	case HEADER_DG_BROWSER_TEAM_LOADMEMBER:
	case HEADER_DG_BROWSER_TEAM_ADD:
	case HEADER_DG_BROWSER_TEAM_REMOVE:
	case HEADER_DG_BROWSER_TEAM_CREATE:
	case HEADER_DG_BROWSER_TEAM_DESTROY:
	case HEADER_DG_BROWSER_TEAM_GOTOCOPY:
	case HEADER_DG_BROWSER_TEAM_GIVEWARP:
		DungeonBrowser(bHeader, m_dwHandle, c_pData);
		break;
#ifdef __AUCTION__
...
void CInputDB::DungeonBrowser(BYTE bHeader, DWORD dwHandle, const char* c_pData)
{
	switch (bHeader)
	{
		case HEADER_DG_BROWSER_TEAM_ADD:
		{
			CBrowserTeam* pTeam = CDungeonBrowserManager::Instance().GetDungeonTeam(*(DWORD*) c_pData);
			c_pData += sizeof(DWORD);
			TPacketBrowserTeamAddDB* pTeamD = (TPacketBrowserTeamAddDB*)c_pData;
			if (pTeam)
			{
				pTeam->AddMember(pTeamD->dwPID, pTeamD->bLevel, pTeamD->strName, pTeamD->bRace);
			}
		}
			break;
		...
		case HEADER_DG_BROWSER_TEAM_REMOVE:
		{
			CBrowserTeam* pTeam = CDungeonBrowserManager::Instance().GetDungeonTeam(*(DWORD*) c_pData);
			c_pData += sizeof(DWORD);
			TPacketBrowserTeamRemove* pTeamD = (TPacketBrowserTeamRemove*)c_pData;
			if (pTeam)
			{
				pTeam->RemoveMember(pTeamD->dwPID);
			}
		}
			break;
		...
		case HEADER_DG_BROWSER_TEAM_UPDATE:
			{
				...
				c_pData += sizeof(DWORD);
				TPacketBrowserTeamUpdateDB* pTeamD = (TPacketBrowserTeamUpdateDB*)c_pData;
				sys_log(0, "HEADER_DG_BROWSER_TEAM_LOADMEMBER: %d , %d , %d", pTeamD->dwPID,dwLeaderID,pTeamD->bState);
				if (pTeamD->bState)
					CDungeonBrowserManager::Instance().DB_P2PLogin(dwLeaderID, pTeamD->dwPID);
				else
					CDungeonBrowserManager::Instance().DB_P2PLogout(dwLeaderID, pTeamD->dwPID);
			}
				break;

		case HEADER_DG_BROWSER_TEAM_DESTROY:
			{
				CDungeonBrowserManager::Instance().DB_DestroyDungeonTeam(*(DWORD*) c_pData);
			}
				break;
		...
		case HEADER_DG_BROWSER_TEAM_GIVEWARP:
		{
			DWORD dwLeaderID = *(DWORD*) c_pData;
			c_pData += sizeof(DWORD);
			CBrowserTeam* pTeam = CDungeonBrowserManager::Instance().GetDungeonTeam(dwLeaderID);
			TPacketBrowserTeamGiveWarp* pTeamD = (TPacketBrowserTeamGiveWarp*)c_pData;
			sys_log(0, "HEADER_DG_BROWSER_TEAM_GIVEWARP: %d , %d , %d", pTeamD->map_index,pTeamD->mapx,pTeamD->mapy);
			if (pTeam)
			{
				pTeam->JoinAll(pTeamD->map_index, pTeamD->mapx, pTeamD->mapy);
			}
		}
			break;
		...
		case HEADER_DG_BROWSER_TEAM_CREATE:
			{
				DWORD dwLeaderID = *(DWORD*) c_pData;
				c_pData += sizeof(DWORD);
				TPacketBrowserTeamCreateDB* pTeamD = (TPacketBrowserTeamCreateDB*)c_pData;
				CDungeonBrowserManager::Instance().DB_CreateTeam(dwLeaderID,pTeamD->bGameIndex,pTeamD->map_index);
			}
			break;
		case HEADER_DG_BROWSER_TEAM_LOADMEMBER:
			{
				CBrowserTeam* pTeam = CDungeonBrowserManager::Instance().GetDungeonTeam(*(DWORD*) c_pData);
				sys_log(0, "HEADER_DG_BROWSER_TEAM_LOADMEMBER: %d", *(DWORD*) c_pData);
				if (pTeam)
				{
					c_pData += sizeof(DWORD);
					WORD wSize = *(WORD*)c_pData;
					c_pData += sizeof(WORD);
					pTeam->LoadMember((TMemberBrowser*)c_pData, wSize);
				}
			}
		break;
