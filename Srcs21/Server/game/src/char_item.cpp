	...
	switch (bodyPart)
	{
		...
		case 2:
         if (g_gm_trade) 
         {
            ...
            {
               ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM's k?nen keinen Laden ?fnen!"));
            }
            else
            {
		...
		case ITEM_TREASURE_KEY:
			{
				...
				{
					ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("¿­¼e·I ¿ⓒ´A ¹°°CAI ¾Æ´N°I °°´U."));
					return false;
				}
				if (item2->GetCount() < 10 && ((item2->GetVnum() >= 102100 || item2->GetVnum() <= 102105) || (item2->GetVnum() >= 102040 || item2->GetVnum() <= 102045)))
				{
					ChatPacket(CHAT_TYPE_TALKING,"Du brauchst mindestens 10 Stuck fur die Herstellung!");
					return false;
				}

				if (item->GetValue(0) == item2->GetValue(0))
				{
					//ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("¿­¼e´A ¸AA¸³ª ¾ÆAIAU AO´A ºIºÐ ±¸CoAI ¾EμC¾u½A´I´U."));
					...
					{
						if (item->GetVnum() == 103001 || item->GetVnum() == 103002)
						{
						...
						for (int i = 0; i < count; i++){
							switch (dwVnums[i])
							{
								case CSpecialItemGroup::GOLD:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("μ· %d ³EA≫ E¹μæCß½A´I´U."), dwCounts[i]);
									break;
								case CSpecialItemGroup::EXP:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU¿¡¼­ ºIAI ½AºnCN ºuAI ³ª¿E´I´U."));
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%dAC °æCeA¡¸| E¹μæCß½A´I´U."), dwCounts[i]);
									break;
								case CSpecialItemGroup::MOB:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU¿¡¼­ ¸o½ºAI°¡ ³ªA¸³μ½A´I´U!"));
									break;
								case CSpecialItemGroup::SLOW:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU¿¡¼­ ³ª¿A ≫¡°￡ ¿￢±a¸| μeAI¸¶½AAU ¿oA÷AI´A ¼Oμμ°¡ ´A·AA³½A´I´U!"));
									break;
								case CSpecialItemGroup::DRAIN_HP:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU°¡ °ⓒAU±a Æø¹ßCI¿´½A´I´U! ≫y¸i·AAI °¨¼OCß½A´I´U."));
									break;
								case CSpecialItemGroup::POISON:
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU¿¡¼­ ³ª¿A ³i≫o ¿￢±a¸| μeAI¸¶½AAU μ¶AI ¿A¸oA¸·I ÆUAy´I´U!"));
									break;
								...
								default:
									...
									{
										if (dwCounts[i] > 1)
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU¿¡¼­ %s °¡ %d °³ ³ª¿O½A´I´U."), item_gets[i]->GetName(), dwCounts[i]);
										else
											ChatPacket(CHAT_TYPE_INFO, LC_TEXT("≫oAU¿¡¼­ %s °¡ ³ª¿O½A´I´U."), item_gets[i]->GetName());
					...
					{
						ChatPacket(CHAT_TYPE_TALKING, LC_TEXT("¿­¼e°¡ ¸AAo ¾E´A °I °°´U."));
						return false;
					}
				}
				else
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
   ...
   if (g_gm_trade)
   {
      if (GetGMLevel() > GM_PLAYER)
      {
         ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM's k?nen keine Items auf dem Boden werfen!"));
         return false;
      }
   }
