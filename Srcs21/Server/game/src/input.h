...
class CInputMain : public CInputProcessor
{
	...
	protected:
		...
		void		DungeonBrowser(LPCHARACTER ch, BYTE header, const char* c_pData);
};
...
protected:
	...
	*/
	...
	void		DungeonBrowser(BYTE bHeader, DWORD dwHandle, const char* c_pData);
...
class CInputP2P : public CInputProcessor
{
	...
	public:
		...
		int			WhisperAll(LPDESC d, const char * c_pData, size_t uiBytes);
		...
		int			Browser(LPDESC d, const char* c_pData, size_t uiBytes);
