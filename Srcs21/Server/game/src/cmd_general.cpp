...
ACMD(do_dungeon_into)
{

	if (CDungeonBrowserManager::Instance().GetDungeonTeam(ch->GetPlayerID()) == NULL){
		ch->ChatPacket(CHAT_TYPE_INFO, "Du bist kein Leader von einem Dungeonteam!");
		return;
	}

	if (ch->m_pkDungeonBrowserEvent)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Der Dungeonport wurde abgebrochen!");
		event_cancel(&ch->m_pkDungeonBrowserEvent);
		return;
	}

	if (!CDungeonBrowserManager::Instance().AllOnline(ch->GetPlayerID()))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nicht alle Spieler des Dungeonteams sind online.");
		return;
	}
	
	if(ch->GetBrowser()->GetDungeonMemberCount() < 3 )
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Du hast weniger als 3 Spieler in deinem Dungeonteam.");
		return;		
	}

	CDungeonBrowserManager::Instance().JoinAlltoDungeon(ch->GetPlayerID());
