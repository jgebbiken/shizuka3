...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	switch (pinfo->type)
	{
		...
		case CHAT_TYPE_PARTY:
			...
			break;
		case CHAT_TYPE_BROWSER:
		{
			if (!ch->GetBrowser())
				ch->ChatPacket(CHAT_TYPE_INFO, "Du bist in keinem Browser Team");
			else
			{
				ch->GetBrowser()->Chat(chatbuf);
			}
		}
			break;
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	if( IS_SET(pkItem->GetAntiFlag(), ITEM_ANTIFLAG_SAFEBOX) )
	{
      if (g_gm_trade)
      {
         ...
         {
            ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GM's k?nen kein Lager benutzen"));
            return;
         }
      }
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<창고> 이 아이템은 넣을 수 없습니다."));
...
void CInputMain::DungeonBrowser(LPCHARACTER ch, BYTE header, const char* c_pData)
{
	switch (header)
	{
		case HEADER_CG_BROWSER_ADD:
		{
			TPacketCGBrowserAdd* p = (TPacketCGBrowserAdd*)c_pData;
			CDungeonBrowserManager::instance().AddMember(ch, p->dwPID);
		}
		break;
		case HEADER_CG_BROWSER_REMOVE:
		{
			TPacketCGBrowserAdd* p = (TPacketCGBrowserAdd*)c_pData;
			CDungeonBrowserManager::instance().RemoveMember(ch, p->dwPID);
		}
			break;
		case HEADER_CG_BROWSER_CREATE:
		{
			TPacketCGBrowserCreate* p = (TPacketCGBrowserCreate*)c_pData;
			CDungeonBrowserManager::instance().CreateTeam(ch, p->bGameIndex);
		}
			break;
		case HEADER_CG_BROWSER_DESTROY:
		{
			TPacketCGBrowserDestroy* p = (TPacketCGBrowserDestroy*)c_pData;
			CDungeonBrowserManager::instance().DestroyDungeonTeam(ch->GetPlayerID());
		}
			break;
	}
}
...
int CInputMain::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
		...
		case HEADER_CG_BROWSER_CREATE:
		case HEADER_CG_BROWSER_DESTROY:
		case HEADER_CG_BROWSER_UPDATE:
			DungeonBrowser(ch, bHeader, c_pData);
			break;
