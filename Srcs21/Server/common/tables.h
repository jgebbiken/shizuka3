...
enum
{
	...
	HEADER_GD_BROWSER_TEAM_ADD = 76,
	HEADER_GD_BROWSER_TEAM_REMOVE = 77,
	HEADER_GD_BROWSER_TEAM_LOAD = 78,
	HEADER_GD_BROWSER_TEAM_UPDATE = 79,
	HEADER_GD_BROWSER_TEAM_CREATE = 80,
	HEADER_GD_BROWSER_TEAM_DESTROY = 81,
	HEADER_GD_BROWSER_TEAM_GOTOCOPY = 82,
	HEADER_GD_BROWSER_TEAM_GIVEWARP = 83,
	...
	HEADER_DG_BROWSER_TEAM_UPDATE		= 182,
	HEADER_DG_BROWSER_TEAM_LOADMEMBER		= 183,
	HEADER_DG_BROWSER_TEAM_ADD = 184,
	HEADER_DG_BROWSER_TEAM_REMOVE = 185,
	HEADER_DG_BROWSER_TEAM_CREATE = 186,
	HEADER_DG_BROWSER_TEAM_DESTROY = 187,
	HEADER_DG_BROWSER_TEAM_GOTOCOPY = 188,
	HEADER_DG_BROWSER_TEAM_GIVEWARP = 189,
...
typedef struct SMemberBrowser
{
	...
	char	strName[CHARACTER_NAME_MAX_LEN + 1];
...
typedef struct SPacketBrowserTeamUpdate
{
	DWORD	dwLeaderPID;
	DWORD	dwPID;
	BYTE	bState;
} TPacketBrowserTeamUpdate;

typedef struct SPacketBrowserTeamUpdateDB
{
	DWORD	dwPID;
	BYTE	bState;
} TPacketBrowserTeamUpdateDB;

typedef struct  SPacketBrowserTeamGiveWarp
{
	DWORD	dwLeaderID;
	long	map_index;
	long	mapx;
	long	mapy;
} TPacketBrowserTeamGiveWarp;

typedef struct SPacketBrowserTeamAdd
{
	DWORD	dwLeaderPID;
	DWORD	dwPID;
	char	strName[CHARACTER_NAME_MAX_LEN + 1];
	int	bLevel;
	BYTE	bRace;
} TPacketBrowserTeamAdd;

typedef struct SPacketBrowserTeamAddDB
{
	DWORD	dwPID;
	char	strName[CHARACTER_NAME_MAX_LEN + 1];
	int	bLevel;
	BYTE	bRace;
} TPacketBrowserTeamAddDB;

typedef struct SPacketBrowserTeamRemove
{
	DWORD	dwLeaderPID;
	DWORD	dwPID;
} TPacketBrowserTeamRemove;

typedef struct SPacketBrowserTeamRemoveDB
{
	DWORD	dwPID;
} TPacketBrowserTeamRemoveDB;

typedef struct SPacketBrowserTeamDestroy
{
	DWORD	dwLeaderPID;
} TPacketBrowserTeamDestroy;

typedef struct SPacketBrowserTeamCreate
{
	DWORD	dwLeaderPID;
	int	bLevel;
	BYTE	bRace;
	char	strName[CHARACTER_NAME_MAX_LEN + 1];
	int	bGameIndex;
} TPacketBrowserTeamCreate;

typedef struct SPacketBrowserTeamCreateDB
{
	int	bGameIndex;
	long	map_index;
} TPacketBrowserTeamCreateDB;
