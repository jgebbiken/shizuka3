...
void CClientManager::__RESULT_PLAYER_DELETE(CPeer *peer, SQLMsg* msg)
{
	...
	if (msg->Get() && msg->Get()->uiNumRows)
	{
		...
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM player.browser_team_member WHERE leader_id = %u", pi->player_id);
		CDBManager::instance().AsyncQuery(queryStr);
		
		snprintf(queryStr, sizeof(queryStr), "DELETE FROM player.dungeon_browser WHERE leader_id = %u", pi->player_id);
		CDBManager::instance().AsyncQuery(queryStr);
