...
void CInputDB::Boot(const char* data)
{
	...
	if (size)
	{
		...
		for (int i = 0; i < size; ++i, ++p)
		{
			CDungeonBrowserManager::Instance().DB_CreateTeam(p->dwLeaderID,p->bGameIndex);
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData)
{
	...
	CDungeonBrowserManager::Instance().P2PLogin(ch->GetPlayerID());
...
void CInputDB::DungeonBrowser(BYTE bHeader, DWORD dwHandle, const char* c_pData)
{
	switch (bHeader)
	{
		case HEADER_DG_BROWSER_TEAM_ADD:
		{
			TPacketBrowserTeamAdd* pTeamD = (TPacketBrowserTeamAdd*)c_pData;
			CBrowserTeam* pTeam = CDungeonBrowserManager::Instance().GetDungeonTeam(pTeamD->dwLeaderPID);
			if (pTeam)
			{
		...
		case HEADER_DG_BROWSER_TEAM_UPDATE:
			{
				TPacketBrowserTeamUpdate* pTeamD = (TPacketBrowserTeamUpdate*)c_pData;
				if (pTeamD->bState)
					CDungeonBrowserManager::Instance().DB_P2PLogin(pTeamD->dwLeaderPID, pTeamD->dwPID);
				else
					CDungeonBrowserManager::Instance().DB_P2PLogout(pTeamD->dwLeaderPID, pTeamD->dwPID);
			}
				break;

		case HEADER_DG_BROWSER_TEAM_DESTROY:
			{
				TPacketBrowserTeamDestroy* pTeamD = (TPacketBrowserTeamDestroy*)c_pData;
				CDungeonBrowserManager::Instance().DB_DestroyDungeonTeam(pTeamD->dwLeaderPID);
			}
				break;

		case HEADER_DG_BROWSER_TEAM_CREATE:
			{
				TPacketBrowserTeamCreate* pTeamD = (TPacketBrowserTeamCreate*)c_pData;
				CDungeonBrowserManager::Instance().DB_CreateTeam(pTeamD->dwLeaderPID,pTeamD->bGameIndex);
			}
			break;
		case HEADER_DG_BROWSER_TEAM_LOADMEMBER:
			{
