...
int CInputP2P::WhisperAll(LPDESC d, const char * c_pData, size_t uiBytes)
{
	TPacketGGNotice * p = (TPacketGGNotice *)c_pData;
...
int CInputP2P::Browser(LPDESC d, const char* c_pData, size_t uiBytes)
{
	TPacketGGBrowser * p = (TPacketGGBrowser *)c_pData;
	uiBytes -= sizeof(TPacketGGBrowser);
	c_pData += sizeof(TPacketGGBrowser);

	CBrowserTeam * g = CDungeonBrowserManager::instance().GetDungeonTeam(p->dwLeaderID);

	switch (p->bSubHeader)
	{
	case BROWSER_SUBHEADER_GG_CHAT:
	{
		if (uiBytes < sizeof(TPacketGGBrowserChat))
			return -1;

		TPacketGGBrowserChat * p = (TPacketGGBrowserChat *)c_pData;

		if (g)
			g->P2PChat(p->szText);

		return sizeof(TPacketGGBrowserChat);
	}

	default:
		sys_err("UNKNOWN BROWSER SUB PACKET");
		break;
	}
	return 0;
}
...
int CInputP2P::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	switch (bHeader)
	{
		...
		case HEADER_GG_BROWSER_TEAM:
			if ((iExtraLen = Browser(d, c_pData, m_iBufferLeft)) < 0)
				return -1;
			break;
