...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	{
		// º¹¼oCO ¼o AOA¸¸e ¹U·I ½I¿o!
		if (pkPVP->Agree(pkChr->GetPlayerID()))
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s´O°uAC ´e°a ½AAU!"), pkChr->GetName());
			pkChr->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("%s´O°uAC ´e°a ½AAU!"), pkVictim->GetName());
			pkChr->SetDuell(true);
			pkVictim->SetDuell(true);
        }
		return;
	}
...
bool CPVPManager::Dead(LPCHARACTER pkChr, DWORD dwKillerPID)
{
	...
	pkChr->SetDuell(false);
	if(CHARACTER_MANAGER::instance().FindByPID(dwKillerPID))
		CHARACTER_MANAGER::instance().FindByPID(dwKillerPID)->SetDuell(false);
	TR1_NS::unordered_set<CPVP*>::iterator it2 = it->second.begin();
