...
namespace quest 
{
	...
	int pc_start_duell(lua_State * L)
	{
		LPCHARACTER newch = CQuestManager::instance().GetCurrentCharacterPtr();
		
		LPCHARACTER newch1 = CHARACTER_MANAGER::instance().FindPC(lua_tostring(L, 1)); 
		if(newch && newch1)
		{
			if(!newch->GetDuell() && !newch1->GetDuell())
			{
				TPacketGCPVP pack;

				pack.bHeader = HEADER_GC_PVP;
				pack.bMode = PVP_MODE_FIGHT;
				pack.dwVIDSrc = (DWORD)newch->GetVID();
				pack.dwVIDDst = (DWORD)newch1->GetVID();

				LPDESC newd = newch->GetDesc();
				newd->Packet(&pack, sizeof(pack));

				TPacketGCPVP pack1;

				pack1.bHeader = HEADER_GC_PVP;
				pack1.bMode = PVP_MODE_FIGHT;
				pack1.dwVIDSrc = (DWORD)newch1->GetVID();
				pack1.dwVIDDst = (DWORD)newch->GetVID();

				LPDESC newd1 = newch1->GetDesc();
				newd1->Packet(&pack1, sizeof(pack1));
	...
	int pc_set_level(lua_State * L)  
	{
		...
		else
		{
			...
			ch->ResetPoint(MINMAX(0, newLevel, PLAYER_MAX_LEVEL_CONST));			
			return 0;
		}
	}
