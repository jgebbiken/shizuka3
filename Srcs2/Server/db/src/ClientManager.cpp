...
void CClientManager::MainLoop()
{
	...
	CDungeonBrowserManager::Instance().Destroy();
...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
	...
	DWORD dwPacketSize = 
		...
		sizeof(WORD) + sizeof(WORD) + sizeof(TBrowserManagerInitial) * CDungeonBrowserManager::Instance().GetBrowserTeamCount() +
#ifdef __AUCTION__
	...
	sys_log(0, "sizeof(TBrowserManagerInitial) + Teams (%d) = %d ", sizeof(TBrowserManagerInitial), CDungeonBrowserManager::Instance().GetBrowserTeamCount());
	//ADMIN_MANAGER
	...
	peer->EncodeWORD(sizeof(TBrowserManagerInitial));
	peer->EncodeWORD(CDungeonBrowserManager::Instance().GetBrowserTeamCount());
	CDungeonBrowserManager::Instance().InitBrowserCore(peer);
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_BROWSER_TEAM_DESTROY:
				CDungeonBrowserManager::Instance().ProcessPacket(peer, dwHandle, header, data);
				break;
...
int CClientManager::AnalyzeQueryResult(SQLMsg * msg)
{
	...
	switch (qi->iType)
	{
		...
		case QID_BROWSER_MEMBER_LOAD:
			CDungeonBrowserManager::Instance().QueryResult(peer, msg, qi->iType);
			break;
	...
	while (pulses--)
	{
		...
		*/
		...
		if (!(thecore_heart->pulse % thecore_heart->passes_per_sec))
		{
			...
			CDungeonBrowserManager::Instance().Update();
